...
export const validationSchema = Joi.object({
  ...
  CARTULAIRE_CONNECTOR_MOCK_PERMISSIONS: Joi.string().default(
    'identity.resolve,auth.verifyPassword,auth.getMfaMethods,auth.startMfa,auth.verifyMfa,auth.registerMfa,auth.disableMfa,auth.requestPasswordReset,auth.resetPassword,claims.map,consent.get,consent.save,consent.revoke,session.revoke,admin.health',
  ),
}).unknown(true)
